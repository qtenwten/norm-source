...
const sectors = {
  restricted: {
    ...
    records: [
      {id:'17B-LM005-POST',type:'ПОСТОПЕРАЦИОННЫЙ ЖУРНАЛ',title:'LM-005 // сигнал после закрытия дела',summary:'Необработанная выгрузка датчиков после официальной нейтрализации Ночного Душнилы.',filename:'17B_LM005_POST_OPERATION.txt',discovery:'DOWNLOAD_LM005_POST',content:`N.O.R.M. // RESTRICTED 17-B\nFILE: LM005_POST_OPERATION\nACCESS: A-1\n\n03:41:18 SIGNAL DETECTED\n03:41:21 SIGNAL DETECTED\n03:41:24 SIGNAL DETECTED\n\nOPERATOR NOTE:\nвероятно остаточный шум\n\nENTITY STATUS:\nNEUTRALIZED -> [REDACTED]\n\nLAST WRITE: NORM-0\nQUERY RESIDUE: 1000-7\n\n--- SYSTEM COMMENT ---\nПовторение — это не ответ.\nСистема распознаёт последовательность.\n`},
  ...
  black: {
    ...
    records:[
      {id:'BLACK-N0-1987',type:'ИСТОЧНИК СЕССИИ',title:'NORM-0 // первый вход',summary:'Выгрузка метаданных неизвестной сессии. Датировка противоречит возрасту системы.',filename:'NORM0_ORIGIN_1987.txt',discovery:'DOWNLOAD_NORM0_ORIGIN',content:`BLACK ARCHIVE // NORM-0\nACCESS: A-2\n\nFIRST LOGIN: 18.11.1987\nCURRENT NORM-OS BUILD: 2026\nCONSISTENCY CHECK: IMPOSSIBLE\nACCOUNT OWNER: NONE\nPROCESS OWNER: NONE\n\nSELF-IDENTIFICATION TOKEN:\n4e4f524d2d30\n\nLAST QUERY:\nWHO ARE YOU\n\nNOTE:\nТокен не зашифрован. Он только записан не тем алфавитом.\n`},
  ...
  vault: {
    level:3, code:'ROOT', eyebrow:'N.O.R.M. // ROOT STORAGE', title:'КОРНЕВОЕ ХРАНИЛИЩЕ', subtitle:'Локальный слой, который не отображается в штатном индексе базы.', discovery:'ROOT_VAULT_VISITED', notice:'Это не финальный уровень. ROOT STORAGE содержит признаки старого системного тома, удалённого из обычной навигации.',
    records:[
      {id:'ROOT-FINAL',type:'НЕАВТОРИЗОВАННЫЙ ФАЙЛ',title:'final.txt // владелец отсутствует',summary:'Файл не числится созданным Н.О.Р.М. и указывает, что следующий слой находится не в архиве, а в самой NORM-OS.',filename:'NORM0_FINAL_HANDSHAKE.txt',discovery:'DOWNLOAD_BLACK_FINAL',content:`THIS FILE WAS NOT CREATED BY N.O.R.M.\n\nTHE ARCHIVE DOES NOT REMEMBER ITS FIRST OPERATOR.\nIT ONLY REMEMBERS THAT SOMEONE WAS HERE BEFORE THE ARCHIVE EXISTED.\n\nNORM-0 // SESSION STILL ACTIVE\nCURRENT OBSERVER: YOU\n\nNEXT HANDSHAKE:\nTHE ARCHIVE IS OVER. INSPECT THE OPERATING SYSTEM.\n\nTRY WHAT AN ADMINISTRATOR WOULD TRY:\nls -la /\n`},
      {id:'ROOT-TRACE',type:'СЛЕД СЕССИИ',title:'trace_0000.log // текущий наблюдатель',summary:'Снимок служебной таблицы после получения A-3. Некоторые поля формируются в момент скачивания.',filename:'SESSION_TRACE_0000.log',discovery:'DOWNLOAD_ROOT_TRACE',dynamic:true},
      {id:'ROOT-MANIFEST',type:'МАНИФЕСТ',title:'manifest // структура скрытого слоя',summary:'Список открытых секторов и признак того, что за ROOT существует несмонтированный legacy volume.',filename:'ROOT_STORAGE_MANIFEST.txt',discovery:'DOWNLOAD_ROOT_MANIFEST',content:`N.O.R.M. // ROOT STORAGE MANIFEST\n\nA-0 PUBLIC DATABASE\nA-1 RESTRICTED 17-B\nA-2 BLACK ARCHIVE\nA-3 ROOT STORAGE\nA-4 [LEGACY VOLUME // NOT MOUNTED]\nA-5 [MIRROR // PRIVILEGED]\n\nSYSTEM NOTE:\nArchive navigation intentionally omits operating-system paths.\nUse the field terminal as a shell, not only as a search box.\n`},
...
export default function ClearanceZone({ zone = 'restricted' }) {
  ...
  useEffect(() => { if (progress.clearance >= config.level) discover(config.discovery) }, [config.discovery, config.level, progress.clearance])
  ...
  const download = (record) => { audio.archive(); downloadText(record, progress); discover(record.discovery) }

  if (!unlocked) return <div className="page clearance-zone clearance-zone--locked"><section className="clearance-denied"><small>{config.eyebrow}</small><strong>ACCESS DENIED</strong><h1>{config.title}</h1><p>Текущая сессия имеет уровень A-{progress.clearance}. Для этого раздела требуется A-{config.level}.</p><div><span>СЕКТОР СУЩЕСТВУЕТ</span><span>СОДЕРЖИМОЕ СКРЫТО</span><span>МАРШРУТ ЗАРЕГИСТРИРОВАН</span></div><button type="button" onClick={() => navigate('/terminal')}>ПЕРЕЙТИ В ТЕРМИНАЛ →</button></section></div>

  return <div className={`page clearance-zone clearance-zone--${zone}`}>
    <header className="clearance-zone__head"><div><small>{config.eyebrow}</small><h1>{config.title}</h1><p>{config.subtitle}</p></div><div className="clearance-zone__badge"><span>СЕКТОР</span><b>{config.code}</b><small>ACCESS A-{config.level} VERIFIED</small></div></header>
    <div className="clearance-zone__notice"><span>⚠</span><p>{config.notice}</p></div>
    <div className="clearance-zone__stats"><span>ДОСТУП <b>A-{progress.clearance}</b></span><span>ФАЙЛОВ <b>{config.records.length}</b></span><span>СКАЧАНО <b>{downloads}</b></span><button type="button" onClick={() => navigate('/terminal')}>FIELD TERMINAL →</button></div>
    <div className="classified-grid">{config.records.map((record,index)=><article className="classified-file" key={record.id} style={{'--delay':`${index*70}ms`}}><div className="classified-file__mark"><span>{String(index+1).padStart(2,'0')}</span><i/></div><small>{record.type}</small><h2>{record.title}</h2><p>{record.summary}</p><dl><dt>INDEX</dt><dd>{record.id}</dd><dt>EXPORT</dt><dd>{record.filename}</dd><dt>CHECKSUM</dt><dd>{progress.discoveries.includes(record.discovery)?'DOWNLOADED':'UNREAD'}</dd></dl><div className="classified-file__actions"><button type="button" data-sound="drawer" onClick={()=>{audio.drawer();setSelected(record)}}>ПРОСМОТРЕТЬ</button><button type="button" data-sound="terminal" onClick={()=>download(record)}>⇩ СКАЧАТЬ ФАЙЛ</button></div></article>)}</div>
    {selected&&<div className="classified-viewer" role="dialog" aria-modal="true" onClick={()=>setSelected(null)}><section onClick={(event)=>event.stopPropagation()}><header><div><small>{config.code} // SECURE PREVIEW</small><b>{selected.id}</b></div><button type="button" onClick={()=>setSelected(null)}>×</button></header><pre>{buildDynamicFile(selected,progress)}</pre><footer><span>Предпросмотр показывает тот же текст, который будет сохранён на компьютер.</span><button type="button" onClick={()=>download(selected)}>⇩ СКАЧАТЬ {selected.filename}</button></footer></section></div>}
  </div>
}
