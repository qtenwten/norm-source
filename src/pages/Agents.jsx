...
const agents = [
  { id:'sen', code:'AG-SEN', name:'СЕН', status:'АКТИВЕН', category:'active', clearance:'A-3', group:'LM', image:'/assets/host_duo_outdoor.png', position:'30%', records:['LM-001 — LM-006','FIELD MEDIA: ДОСТУПНО','ПОЛЕВАЯ ГРУППА: LM'], note:'Часть полей не синхронизирована с бумажным архивом.' },
  { id:'grig', code:'AG-GRIG', name:'ГРИГ', status:'АКТИВЕН', category:'active', clearance:'A-3', group:'LM', image:'/assets/host_duo_outdoor.png', position:'74%', records:['LM-001 — LM-006','FIELD MEDIA: ДОСТУПНО','ПОЛЕВАЯ ГРУППА: LM'], note:'Часть полей не синхронизирована с бумажным архивом.' },
  { id:'04', code:'AG-04', name:'████████', status:'ДЕАКТИВИРОВАН', category:'archive', clearance:'B-2', group:'██', records:['ПОСЛЕДНЕЕ ДЕЛО: LM-0██','ПРОТОКОЛ 17-Б','ПОЛЕВЫЕ МАТЕРИАЛЫ: ИЗЪЯТЫ'], note:'Доступ прекращён административным решением. Причина скрыта уровнем допуска.' },
  { id:'09', code:'AG-09', name:'██████ █.', status:'УТРАЧЕН', category:'lost', clearance:'B-4', group:'██', records:['ПОСЛЕДНИЙ СИГНАЛ: --:--:--','ПОСЛЕДНЕЕ ДЕЛО: LM-0██','СТАТУС ПОИСКА: ЗАКРЫТ'], note:'В реестре отсутствует запись о завершении службы. Файл восстановлен частично.' },
  { id:'13', code:'AG-13', name:'НЕТ ДАННЫХ', status:'ФАЙЛ УДАЛЁН', category:'deleted', clearance:'████', group:'██', records:['КАРТОЧКА СОТРУДНИКА: НЕДОСТУПНА','БИОМЕТРИЯ: НЕТ ДАННЫХ','СВЯЗАННЫЕ ДЕЛА: ███████'], note:'Цифровая копия была удалена до создания текущей версии NORM-OS.' },
  { id:'02', code:'AG-02', name:'НЕ ЧИСЛИТСЯ', status:'НЕСООТВЕТСТВИЕ РЕЕСТРУ', category:'anomaly', clearance:'?', group:'—', records:['ДАТА СОЗДАНИЯ: НЕДОСТУПНА','ИСТОЧНИК: UNKNOWN NODE','СВЯЗАННЫЕ ДЕЛА: 0'], note:'Система не находит сотрудника с таким номером, однако карточка появляется снова после удаления.' },
]
...
export default function Agents() {
  ...
  const [selectedId,setSelectedId]=useState('sen')
  const visibleAgents=useMemo(()=>agents.filter(a=>matchesFilter(a,filter)),[filter])
  const selected=agents.find(a=>a.id===selectedId)||agents[0]

  const selectAgent=(agent)=>{setSelectedId(agent.id);['deleted','anomaly','lost'].includes(agent.category)?audio.glitch():audio.stamp()}
