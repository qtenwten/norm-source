...
export default function LegacyCases(){
  ...
  useEffect(()=>{if(recovered&&routeRestored&&progress.clearance>=4) discover('LEGACY_CASES_VISITED')},[recovered,routeRestored,progress.clearance])
  useEffect(()=>{if(allEvidence) discover('LEGACY_EVIDENCE_COMPLETE')},[allEvidence])

  if(progress.clearance<4 || !recovered || !routeRestored){
    const message=!recovered?'Система не может связать дела с персоналом. Сначала восстановите личные дела AG-04, AG-09 и AG-13 в GHOSTFS.':'Личные дела восстановлены, но визуальный маршрут удалён из таблицы NORM-OS. Найдите orphaned route в MIRROR и восстановите его служебным reindex.'
    return <div className="page legacy-cases legacy-cases--locked"><section className="clearance-denied"><small>N.O.R.M. // ORPHANED CASE ROUTE</small><strong>ROUTE OFFLINE</strong><h1>ВОССТАНОВЛЕННЫЕ ДЕЛА</h1><p>{message}</p><div><span>3 PERSONNEL FILES REQUIRED</span><span>MIRROR ROUTE TABLE</span><span>REINDEX REQUIRED</span></div><button type="button" onClick={()=>{sessionStorage.setItem('norm-terminal-prefill',recovered?'ls -la /mirror':'tree /mnt/ghost/personnel');navigate('/terminal')}}>FIELD TERMINAL →</button></section></div>
  }
  ...
  return <div className="page legacy-cases">
    <header className="legacy-head"><div><small>N.O.R.M. // GHOSTFS // ORPHANED CASE TABLE</small><h1>ВОССТАНОВЛЕННЫЕ ДЕЛА</h1><p>Три внутренних расследования старых подразделений Н.О.Р.М. Они не являются публичными выпусками группы LM и не заменяют события LM-005/LM-006 — это скрытая история самой организации.</p></div><div className="legacy-head__badge"><span>ROUTE</span><b>/legacy-cases</b><small>REINDEXED FOR CURRENT SESSION</small></div></header>
