...
export default function Agents() {
  ...
  return <div className="page agents-registry-page">
    ...
    <div className="registry-grid">{visibleAgents.map((agent,index)=><button type="button" data-agent={agent.id} className={`registry-card registry-card--${agent.category} ${selected.id===agent.id?'is-selected':''}`} onClick={()=>selectAgent(agent)} style={{'--delay':`${index*70}ms`}} key={agent.code}><AgentPortrait agent={agent}/><div className="registry-card__copy"><small>{agent.code}</small><strong>{agent.name}</strong><span>{agent.status}</span><i>ДОПУСК: {agent.clearance}</i></div><em>ОТКРЫТЬ ДОСЬЕ →</em></button>)}</div>
    <section data-agent={selected.id} className={`agent-expanded agent-expanded--${selected.category}`} key={selected.id}>
