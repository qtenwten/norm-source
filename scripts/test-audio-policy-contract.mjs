...
expect(!shell.includes('type="range"'), 'volume range control must not exist in the shell')
expect(!shell.includes('volume-slider'), 'legacy volume slider class is still rendered')
expect(!shell.includes('volume-console__scale'), 'legacy volume scale is still rendered')
expect(!shell.includes('100 // LOCKED'), 'legacy volume description is still rendered')
...
if (!process.exitCode) {
  console.log('AUDIO POLICY CONTRACT: OK // fixed 100% gain + explicit mute only + no slider UI')
}
